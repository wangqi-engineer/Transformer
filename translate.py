...
def main():
    ...
    checkpoint = torch.load(opt.model_dir, weights_only=False)
    ...
    log.info(f'Start to tokenize test data. Please waiting soon...')
    start = time.time()
    tokenizer = Tokenizer(data['vocab'])
    test_src_tensor, test_trg_tensor = tokenizer.tokenize(data['test'])
    duration = time.time() - start
    log.info(f'[Info] Tokenizer test data finished with duration {duration:.2f}s')
    ...
    for src, trg in tqdm(test_dataloader, desc=desc, mininterval=2, leave=False):
        ...
        with open(output_file, 'w', encoding='utf-8') as file:
            for idx in range(src.size(0)):
                ...
                log.info(f'[SRC] {src_sentence}')

                file.write(f'[PRD] {pred_sentence}\n')
                log.info(f'[PRD] {pred_sentence}')

                file.write(f'[TRG] {trg_sentence}\n')
                log.info(f'[TRG] {trg_sentence}')
